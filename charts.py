...
def render_chart(template: str, data: list[dict[str, float]]):
    ...
    html_rendered = string.Template(html_template).substitute(data=json.dumps(data), techMap=json.dumps(TECH_MAP))
...
def electricity_import(scenario: str):
    ...
    export_df = scalars[
        (scalars["var_name"] == "flow_in_electricity")
        & (scalars["tech"] == "export")
        & (scalars["var_value"] > 0)
    ][["name", "var_value"]]
...
def optimized_capacities(scenario: str):
    ...
    merged_df.loc[
        merged_df["capacity_potential"] == float("inf"), "capacity_potential"
    ] = 0
    merged_df.loc[
        merged_df["capacity_potential"] == float("-inf"), "capacity_potential"
    ] = 0
    ...
    merged_df = merged_df[
        ...
        & (
            merged_df["capacity_potential"].notna() & merged_df["capacity_potential"]
            > 0
        )
    ]
...
def generation_consumption_per_sector(scenario: str):
    ...
    x1 = (
        int(
            scalars.loc[(scalars["var_name"] == "flow_in_electricity"), "var_value"]
            .sum()
            .sum()
            / 1000
        )
    ...
    y2 = (
        scalars.loc[
            (scalars["var_name"] == "flow_out_heat_low_decentral")
            & (scalars["var_value"] > 0),
            ["name", "var_value"],
        ]
        .groupby("name")
    ...
    x2 = (
        int(
            scalars.loc[
                (scalars["var_name"] == "flow_in_heat_low_decentral"), "var_value"
            ]
            .sum()
            .sum()
            / 1000
    ...
    y3 = (
        scalars.loc[
            (scalars["var_name"] == "flow_out_heat_low_central")
            & (scalars["var_value"] > 0),
            ["name", "var_value"],
        ]
        .groupby("name")
    ...
    x3 = (
        int(
            scalars.loc[
                (scalars["var_name"] == "flow_in_heat_low_central"), "var_value"
            ]
            .sum()
            .sum()
            / 1000
    ...
    x4 = (
        int(
            scalars.loc[(scalars["var_name"] == "flow_in_heat_high"), "var_value"]
            .sum()
            .sum()
            / 1000
        )
...
def self_generation_imports(scenario: str):
    ...
    y2_df = scalars[
        (scalars["var_name"] == "flow_out_electricity") & (scalars["tech"] == "import")
    ]
